...
| 파일 | 출처 기록 | 배포 시 유의 사항 |
|---|---|---|
| `assets/new-year-card/new-year-minhwa-background.png` | 저장소 관리자가 GPT 계열 이미지 생성 모델로 생성한 이미지 | 외부 스톡·원본 이미지를 가져온 것이 아니라는 관리자의 출처 기록이다. 정확한 모델·계정의 이용 약관과 최종 사용 권리는 배포자가 별도로 확인하며, 이 기록은 제3자 권리 비침해를 보증하지 않는다. |

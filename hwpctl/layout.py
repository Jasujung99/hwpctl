...
줄 수는 HangulCanvas가 조판 줄 이동과 KeyIndicator로 실측한다. 다만 한/글 2022
Automation에는 문자열의 조판 폭을 직접 반환하는 API가 없어, 줄바꿈을 없앨 목표
너비는 글자 크기와 유니코드 문자 폭으로 추정한다. 이 추정치는 한 번에 현재 열의
1.6배, 본문 폭의 45%를 넘지 않게 제한한다.
"""
